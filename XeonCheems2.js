...
case 'pantun':
									...
									break
case 'mess': case 'chat': case 'pesanke':{
if (isQuotedMsg) {
sendBut(quotedMsg.sender, q, `Message From @${sender.split('@')[0]}`, [{buttonId: `${prefix}bacaa ${sender} ${quotedMsg.sender}`, buttonText: {displayText: 'Tandai telah dibaca'}, type: 1}], fgif)
reply(`Pesan Terkirim`)
} else {
let nomm = q.split("|")[0].replace(/[^0-9]/g, '')+'@s.whatsapp.net'
sendBut(nomm, q.split("|")[1], `Message From @${sender.split('@')[0]}`, [{buttonId: `${prefix}bacaa ${sender} ${nomm}`, buttonText: {displayText: 'Tandai telah dibaca'}, type: 1}], fgif)
reply(`Pesan Terkirim`)
}
}
break
