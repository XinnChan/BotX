...
require('./settings')
const { default: XinnConnect, useSingleFileAuthState, DisconnectReason, fetchLatestBaileysVersion, generateForwardMessageContent, prepareWAMessageMedia, generateWAMessageFromContent, generateMessageID, downloadContentFromMessage, makeInMemoryStore, jidDecode, proto } = require("@adiwajshing/baileys")
...
async function connectKeWA() {
    ...
    // anticall auto block
    xinn.ws.on('CB:call', async (json) => {
    const callerId = json.content[0].attrs['call-creator']
    if (json.content[0].tag == 'offer') {
    let pa7rick = await xinn.sendContact(callerId, global.owner)
    xinn.sendMessage(callerId, { text: `Automatic Block System!\nDon't Call Bot!\nPlease Ask Or Contact The Owner To Unblock You!`}, { quoted : pa7rick })
    await sleep(8000)
    await xinn.updateBlockStatus(callerId, "block")
    }
    })
    ...
    xinn.ev.on('messages.upsert', async chatUpdate => {
        ...
        m = smsg(xinn, mek, store)
        require("./XinnChan")(xinn, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
    ...
    // Group Update
    xinn.ev.on('groups.update', async pea => {
       //console.log(pea)
    // Get Profile Picture Group
       try {
       ppgc = await xinn.profilePictureUrl(pea[0].id, 'image')
       } catch {
       ppgc = 'https://shortlink.xinnarridho.my.id/rg1oT'
       }
       let wm_fatih = { url : ppgc }
       if (pea[0].announce == true) {
       xinn.send5ButImg(pea[0].id, `「 Group Settings Changed 」\n\nThe Group Has Been Closed By Admin, Now Only Admin Can Send Messages !`, `Group Settings Change Message`, wm_fatih, [])
    ...
    xinn.ev.on('group-participants.update', async (anu) => {
        console.log(anu)
        try {
            let metadata = await xinn.groupMetadata(anu.id)
            let participants = anu.participants
            for (let num of participants) {
                // Get Profile Picture User
                try {
                    ppuser = await xinn.profilePictureUrl(num, 'image')
                } catch {
                ...
                //Get Profile Picture Group\\
                try {
                    ppgroup = await xinn.profilePictureUrl(anu.id, 'image')
                } catch {
...
🐶✑ I'm Not Sure If It Was A Goodbye Charm, But It Was Fun While It Lasted 😌✨` })
	...
	xinn.sendMessage(jid, { contacts: { displayName: `${list.length} Contact`, contacts: list }, ...opts }, { quoted })
    }
    ...
    xinn.ev.on('connection.update', async (update) => {
        ...
        let reason = new Boom(lastDisconnect?.error)?.output.statusCode
            ...
            else if (reason === DisconnectReason.connectionClosed) { console.log("🦄Connection closed, reconnecting...."); connectKeWA(); }
            else if (reason === DisconnectReason.connectionLost) { console.log("🦄Connection Lost from Server, reconnecting..."); connectKeWA(); }
            ...
            else if (reason === DisconnectReason.restartRequired) { console.log("🦄Restart Required, Restarting..."); connectKeWA(); }
            else if (reason === DisconnectReason.timedOut) { console.log("🦄Connection TimedOut, Reconnecting..."); connectKeWA(); }
            else xinn.end(`🦄Unknown DisconnectReason: ${reason}|${connection}`)
        }
        console.log('Connected...', update)
    })

    xinn.ev.on('creds.update', saveState)
     ...
     * @returns
     */
    xinn.send5ButImg = async (jid , text = '' , footer = '', img, but = [], options = {}) =>{
        let message = await prepareWAMessageMedia({ image: img }, { upload: xinn.waUploadToServer })
        ...
        imageMessage: message.imageMessage,
            ...
            xinn.relayMessage(jid, template.message, { messageId: template.key.id })
    }
     ...
     * @returns 
     */
    xinn.sendTextWithMentions = async (jid, text, quoted, options = {}) => xinn.sendMessage(jid, { text: text, contextInfo: { mentionedJid: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net') }, ...options }, { quoted })
     ...
     * @returns 
     */
    xinn.sendMedia = async (jid, path, fileName = '', caption = '', quoted = '', options = {}) => {
       ...
       await xinn.sendMessage(jid, { [type]: { url: pathFile }, caption, mimetype, fileName, ...options }, { quoted, ...options })
